  ...
  try {
    ...
    if (response && (response.success || response.usuario || response.role)) {
      ...
      console.log("[v0] Sesión guardada:", session)
        ...
        localStorage.setItem("session_" + rol, JSON.stringify(session))
        sessionStorage.setItem("tabSession", JSON.stringify(session))
...
// Función de logout — solo cierra esta pestaña, no afecta otras
function logout() {
...
function checkAuth(requiredRole) {
  ...
  const sessionData = tabData || localStorage.getItem("session")
...
function getSession() {
  const tabData = sessionStorage.getItem("tabSession")
  if (tabData) return JSON.parse(tabData)
